   ...
   Stack<Token> myStack = new Stack<>() ;
        ...
        for(int i = 0 ; i <infixExpression.size() ; i ++ ){
            ...
            switch(infixExpression.get(i).mType){
                case OPERATOR :{
                     op = (Operator) infixExpression.get(i) ;
               while(!myStack.empty() && myStack.lastElement().mType != Token.Type.DELIMETER){
               os = (Operator) myStack.lastElement() ;
               if(op.mPrecedence < os.mPrecedence || (op.mPrecedence == os.mPrecedence && op.mAssociativity ==Operator.Associativity.ASSOC_LEFT )){
                   postfixExpression.add(myStack.lastElement());
                   myStack.pop() ;
                ...
                myStack.push(infixExpression.get(i)) ;
                break ;
                }
                case DELIMETER :{
                     del = (Delimeter) infixExpression.get(i ) ;
                   if(del.mMark == '('){
                       myStack.push(infixExpression.get(i));
                   }else{
                       if(del.mMark == ')'){
                           Token it ;
                           while(!myStack.empty()){
                               it = myStack.lastElement();
                               myStack.pop() ;
                               if(it.mType != Token.Type.DELIMETER){
                ...
                case FUNCTION:{
                     myStack.add(infixExpression.get(i)) ;
                     break ;
                }
            }
